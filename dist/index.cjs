...
const isCodegenConfig = (filePath, configPath) => {
  if (!configPath)
    return false;
  return vite.normalizePath(filePath) === vite.normalizePath(configPath);
};
...
async function getSchemaPaths(context) {
  ...
  if (!schemas.length)
    return [];
  return schemas.filter((schema) => typeof schema === "string").filter(Boolean).map((schema) => node_path.resolve(schema)).map(vite.normalizePath);
}
